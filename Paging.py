...
import sys, os, socket, platform, serial, subprocess, vlc, binascii, re, threading
from _thread import *
...
#Bagic veriable
osPlatform = None;tcpServerPort = 8888;serialBaud = 115200;em_Set_State = None; clientServerIP=None; clientServerPort=None; LocalID=None; bssLevelPresence = ['0']*48;udpLogDest = ('172.28.242.40',9999)

class Main_UI(QMainWindow, Ui_MainWindow):
    tcpServerReturnString = Signal(str);tcpServerReset = Signal();serialReturnString = Signal(str);setSerialBaudChange = Signal(str);
    player_play = Signal(int);player_pause = Signal();player_stop = Signal();bssHiqnetString = Signal(str);
    ...
    def __init__(self):
        ...
        self.tcpServerNetworkStatus = False;self.ind_Streaming_State = False
        self.tcpServerAddress = ['0', '192.168.1.199','255.255.255.0','0.0.0.0','8888','127.0.0.1','12302','1']
        self.ip = ['0']*4;self.nm = ['0']*4;self.gw = ['0']*4;self.clientIp = ['0']*4
        self.btn_State_FileName = ['ButtonState_RG1.txt','ButtonState_RG2.txt','ButtonState_RG3.txt','ButtonState_RG4.txt']
        self.btn_Name_FileName = ['ButtonName_RG1.txt','ButtonName_RG2.txt','ButtonName_RG3.txt','ButtonName_RG4.txt']
        self.btn_Name = [self.lineEdit_Setup_Buttons_Name_1_,self.lineEdit_Setup_Buttons_Name_2_,self.lineEdit_Setup_Buttons_Name_3_,self.lineEdit_Setup_Buttons_Name_4_]
        self.btn_Ui = [self.btn_RG1_, self.btn_RG2_, self.btn_RG3_, self.btn_RG4_]
        ...
        tcpServerPort = int(self.tcpServerAddress[4]); clientServerIP = self.tcpServerAddress[5]; clientServerPort = self.tcpServerAddress[6]; LocalID = self.tcpServerAddress[7]
        ...
        self.TcpSoketServer.TCPServerConnectClient.connect(self.serverConnectClient)
        ...
        #Popup Setup
        self.qdialog = QDialog(); self.qdialog.Qui = Ui_DialogQuestion(); self.qdialog.Qui.setupUi(self.qdialog)
        self.idialog = QDialog(); self.idialog.Qui = Ui_DialogInfo(); self.idialog.Qui.setupUi(self.idialog)
        self.emdialog = QDialog(); self.emdialog.Qui = Ui_DialogEM(); self.emdialog.Qui.setupUi(self.emdialog)
        self.touch4bayDialog = QDialog(); self.touch4bayDialog.Qui = Ui_DialogKeypad_ip(); self.touch4bayDialog.Qui.setupUi(self.touch4bayDialog)
        self.touch1bayDialog = QDialog(); self.touch1bayDialog.Qui = Ui_DialogKeypad_Port(); self.touch1bayDialog.Qui.setupUi(self.touch1bayDialog)
        self.touch4bay_text_ip = [self.touch4bayDialog.Qui.text_Ip1,self.touch4bayDialog.Qui.text_Ip2,self.touch4bayDialog.Qui.text_Ip3,self.touch4bayDialog.Qui.text_Ip4]

        self.setSerialfrom_()   #Serial Port Setup
        self.buttonNameLoad_()  #Button Name

        self.TcpSoketServer.start(); print('TCP Server Loading Completed')
        self.serial_BssMeterRT.start()
        self.udpSendBssMeter.start()
        self.audioStreamReceiver.start(); print('Audio Stream Receiver Ready')
        self.em_Alarm.start()
        ...
        try:
            ...
            start_new_thread(self.udpLogReturn, (LocalID,'Paging Control -- USB Serial 디바이스 없습니다.'))

        start_new_thread(self.udpLogReturn, (LocalID,'Paging Control -- 시스템이 시작되었습니다.'))
        #UI Start
        self.show()
        #Button State Reload
        sleep(1)
    ...
    def udpLogReturn(self, localId, RtString):
        try:
            ...
            self.udpLogRt.sendto('{},{}'.format(localId, RtString).encode('cp949'),udpLogDest)
            self.udpLogRt.close()
    ...
    def em_Alarm_Set(self,state,pinNum):
        if state == 1:
            start_new_thread(self.udpLogReturn, (LocalID, 'Paging Control -- 비상이 발생하였습니다.PIN {}'.format(pinNum)))
            #dialog
            if self.em_Popup == False:
                try:
                    start_new_thread(self.udpReturnThread, ('e,{},1,!'.format(LocalID), ))
                except:
                    self.statusBar().showMessage('네트워크가 활성화 되지 않았습니다.',5000)
                ...
                if self.emdialog.exec_():
                    self.em_Popup = False
                    start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- 지점에서 비상 상황을 확인 하였습니다.'))
        elif state == 0:
            start_new_thread(self.udpLogReturn, (LocalID, 'Paging Control -- 비상이 해제되었습니다.PIN {}'.format(pinNum)))
            start_new_thread(self.udpReturnThread, ('e,{},0,!'.format(LocalID), ))
        elif state == 2:
            start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- 비상 감지 시스템에 장애가 발생 하였습니다.'))
        elif state == 3:
            start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- 비상 감지 시스템을 시작 하였습니다.'))
    #Touch Input
    @Slot()
    ...
    def touchInput_ServerPort(self):
        if self.ipTouchInpusStatus == 0:
            ...
            self.touch1bayDialog.Qui.text_Port.setText(self.lineEdit_Server_Port.text())
            if self.touch1bayDialog.exec_():
                self.lineEdit_Server_Port.setText(self.touch1bayDialog.Qui.text_Port.text())
                self.ipTouchInpusStatus = 0
            else:
    ...
    def touchInput_ClientPort(self):
        if self.ipTouchInpusStatus == 0:
            ...
            self.touch1bayDialog.Qui.text_Port.setText(self.lineEdit_Client_Port.text())
            if self.touch1bayDialog.exec_():
                self.lineEdit_Client_Port.setText(self.touch1bayDialog.Qui.text_Port.text())
                self.ipTouchInpusStatus = 0
            else:
    ...
    def touchInput_LocalID(self):
        if self.ipTouchInpusStatus == 0:
            ...
            self.touch1bayDialog.Qui.text_Port.setText(self.lineEdit_Client_ID.text())
            if self.touch1bayDialog.exec_():
                self.lineEdit_Client_ID.setText(self.touch1bayDialog.Qui.text_Port.text())
                self.ipTouchInpusStatus = 0
            else:
    ...
    def bssReoot_function(self):
        ...
        if self.qdialog.exec_():
            ...
            try:
                self.bssSetString = hiqnet_checksum('88',self.hiqnetNodeAddr,1,1)
                self.bssHiqnetString.emit(self.bssSetString)
                self.statusBar().showMessage('BSS BLU DSP가 재부팅 됩니다. 최대 5분정도 소요됩니다.',5000)
                self.udpLogReturn(LocalID, 'Paging Control -- BSS DSP가 Paging Control에 의해서 재부팅 되었습니다.')
            except:
                self.udpLogReturn(LocalID,'Paging Control -- BSS장비와의 통신에 문제가 발생했습니다.')
    ...
    def bssLocalMic_Btn(self):
        ...
        if self.setFunctionButton1.isChecked():
            ...
            self.hiqnetNodeAddr = ['20',self.hiqnet_localid,'03','00','07','01']
            self.bssSetString = hiqnet_checksum('88',self.hiqnetNodeAddr,33,0)
        else:
            self.hiqnetNodeAddr = ['20',self.hiqnet_localid,'03','00','07','01']
            self.bssSetString = hiqnet_checksum('88',self.hiqnetNodeAddr,33,1)

        try:
            self.bssHiqnetString.emit(self.bssSetString)
            if self.setFunctionButton1.isChecked():
                self.udpLogReturn(LocalID,'Paging Control -- 지점 마이크 사용이 시작 되었습니다.')
                self.statusBar().showMessage('지점 마이크 사용이 시작 되었습니다.',5000)
            else:
                self.udpLogReturn(LocalID,'Paging Control -- 지점 마이크 사용이 종료 되었습니다.')
    ...
    def bssFuction_Btn(self):
        ...
        if self.setFunctionButton2.isChecked():
            #type,nodeAddr,nodeId,parameterValue
            self.hiqnetNodeAddr = ['20',self.hiqnet_localid,'03','00','07','01']
            self.bssSetString = hiqnet_checksum('88',self.hiqnetNodeAddr,35,0)
        else:
            self.hiqnetNodeAddr = ['20',self.hiqnet_localid,'03','00','07','01']
            self.bssSetString = hiqnet_checksum('88',self.hiqnetNodeAddr,35,1)

        try:
            self.bssHiqnetString.emit(self.bssSetString)
            if self.setFunctionButton1.isChecked():
                self.udpLogReturn(LocalID,'Paging Control -- 지점 이벤트 기능이 활성화 되었습니다.')
                self.statusBar().showMessage('지점 이벤트 기능이 활성화 되었습니다.',5000)
            else:
                self.udpLogReturn(LocalID,'Paging Control -- 지점 이벤트 기능이 종료 되었습니다.')
    ...
    def streaming_Play_On(self, state):
        if state == True:
            self.Main_Menu_Indcators[0].setChecked(True); self.ind_Streaming_State = True
        else:
            self.Main_Menu_Indcators[0].setChecked(False); self.ind_Streaming_State = False
        ...
        for group in range(4):
            with open(self.btn_Name_FileName[group],'w') as file:
                for i in range(24):
                    ...
                    self.btn_Ui[group][i].setText(self.btn_Name[group][i].text())
        self.statusBar().showMessage('버튼 이름을 저장 하였습니다.',5000)
        start_new_thread(self.udpLogReturn,(LocalID,'Paging Control -- 릴레이 버튼 이름이 변경되었습니다.'))
            ...
            with open(self.btn_State_FileName[group],'w') as file:
                for i in range(24):
                    if self.btn_Ui[group][i].isChecked():
                        ...
                        file.write('0\n')
        start_new_thread(self.udpLogReturn,(LocalID,'Paging Control -- 버튼 상태값이 저장되었습니다.'))
                ...
                with open(self.btn_State_FileName[group], 'r') as file:
                    ...
                    for i in range(24):
                        if line[i] == '1':
                            ...
                            start_new_thread(self.udpReturnThread,('r,{},{},{},1,!'.format(LocalID, group+1, i+1),))
                        else:
                            self.btn_Ui[group][i].setChecked(False);
                            start_new_thread(self.udpReturnThread,('r,{},{},{},0,!'.format(LocalID, group+1, i+1),))
            if self.serialMode == 0:
                self.BleuRg_Action(0)
            elif self.serialMode == 1:
                self.DRG116_Action(0) #DRG116 상태 갱신
            start_new_thread(self.udpLogReturn,(LocalID,'Paging Control -- 버튼 상태값 불러오기를 실행하였습니다.'))
    ...
    @Slot()
    def setHome(self):
        self.Main_Menu_Buttons[0].setChecked(True); self.Main_View.setCurrentIndex(0)
    ...
    def serialModeSaveToFile(self):
        ...
        if self.qdialog.exec_():
            ...
            start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- Serial Mode 설정이 변경되었습니다.'))
    ...
    def serverIpAddressSave(self):
        ...
        self.qdialog.Qui.Main_Message.setText("시스템 IP 주소를 저장하시겠습니까?\n재부팅 이후 적용 됩니다.")

        if self.qdialog.exec_():
            try:            
                self.change_IP_Address(True)                
                start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- IP가 변경되었습니다. {}'.format(' '.join(self.tcpServerAddress)),))
                self.statusBar().showMessage('  Paging IP 설정이 저장 되었습니다. 리부팅 이후 적용됩니다.',5000)
            except:
                self.change_IP_Address(True)                
                start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- IP 설정 저장 중 문제가 발생하였습니다.'))
    ...
    def change_IP_Address(self, ipChange):
        if ipChange == True:
            for i in range(4):
                if self.lineEdit_IP[i].text() == '': self.ip[i] = '0'
                else: self.ip[i] = self.lineEdit_IP[i].text()

                if self.lineEdit_NM[i].text() == '' and i != 3: self.nm[i] = '255'
                elif self.lineEdit_NM[i].text() == '' and i==3: self.nm[i] = '0'
                else: self.nm[i] = self.lineEdit_NM[i].text()

                if self.lineEdit_GW[i].text() == '': self.gw[i] = '0'
                else: self.gw[i] = self.lineEdit_GW[i].text()

                self.clientIp[i] = self.lineEdit_Client[i].text()

            self.tcpServerAddress[1] = '.'.join(self.ip)
            if self.tcpServerAddress[1] == '0.0.0.0': self.tcpServerAddress[1] = '192.168.1.199'
            self.tcpServerAddress[2] = '.'.join(self.nm)
            self.tcpServerAddress[3] = '.'.join(self.gw)
            if self.lineEdit_Server_Port.text() == '': self.tcpServerAddress[4] = '8888'
            else: self.tcpServerAddress[4] = self.lineEdit_Server_Port.text()
            ...
            try:
                with open('/etc/dhcpcd.conf','w') as file:
                    file.write('interface eth0\nstatic ip_address={}/24\nstatic routers={}'.format(self.tcpServerAddress[1],self.tcpServerAddress[3]))
            except:
                pass
        else:
            try:
    ...
    def serverConnectClient(self,state):
        if state == True:
            self.Main_Menu_Indcators[2].setChecked(True); self.tcpServerNetworkStatus = True
            start_new_thread(self.udpLogReturn,(LocalID, '네트워크 클라이언트가 연결 되었습니다.'))
            self.statusBar().showMessage('네트워크 클라이언트가 연결되었습니다.',5000)

        else:
            self.Main_Menu_Indcators[2].setChecked(False); self.tcpServerNetworkStatus = False
            start_new_thread(self.udpLogReturn(LocalID, '네트워크 클라이언트가 연결이 해제 되었습니다.'))
        ...
        try:
            if data == 'recall':
                ...
                if self.set_RG_Btn[0] == 'r' and len(self.set_RG_Btn) == 4:
                    self.relay_Btn_Act_('Remote',int(self.set_RG_Btn[1]),int(self.set_RG_Btn[2]),int(self.set_RG_Btn[3]))
                if self.set_RG_Btn[0] == 'play':
                    self.player_play.emit(int(self.set_RG_Btn[1])-1)
                elif self.set_RG_Btn[0] == 'stop':
                    self.player_stop.emit()
        except:
            self.tcpServerReturnString.emit('Error Message {0}'.format(len(self.rg_btn_Status)))
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Escape:
            start_new_thread(self.udpLogReturn,(LocalID,'Paging Control -- 어플리케이션이 종료 되었습니다.'))
            sys.exit()  
    #System reboot
    @Slot()    
    def systemReboot(self):
        ...
        #ok
        if self.qdialog.exec_():
            start_new_thread(self.udpLogReturn,(LocalID,'Paging Control -- 시스템을 재부팅합니다.'))
            subprocess.run('sudo reboot', shell=True)

    @Slot()
    def main_Page_Change(self):
        self.mainPageNum = self.buttonGroup_MainMenu.checkedId();self.Main_View.setCurrentIndex(self.mainPageNum)
    @Slot(int)
    ...
    def btn_Clicked(self, id, btnNum):
        if btnNum == 24: self.relayChannel = 0; self.relayValue = 1
        elif btnNum == 25: self.relayChannel = 0; self.relayValue = 0
        else:
    ...
    def relay_Btn_Act_(self, _from, _relayGroup, _relayChannel, _relayValue):
        ...
        #전체 동작
            ...
            if _relayValue == 1:
                for i in range(24): self.btn_Ui[_relayGroup-1][i].setChecked(True)
            elif _relayValue == 0:
                for i in range(24): self.btn_Ui[_relayGroup-1][i].setChecked(False)
        ...
        #서버 전달
        start_new_thread(self.udpLogReturn,(LocalID, 'Paging Control -- {} ID {} CH {} Value {}'.format(_from, _relayGroup, _relayChannel, _relayValue)))
        if _from == 'Local':
            start_new_thread(self.udpReturnThread, ('r,{},{},{},{},!'.format(LocalID, _relayGroup, _relayChannel, _relayValue), ))
            start_new_thread(self.Communicate_State,(True,))
    #BLUE RELAY 동작
    def BleuRg_Action(self, groupId):
        if groupId == 0:
            for group in range(4):
                for i in range(24):
                    if i == 0:
                        if self.btn_Ui[group][i].isChecked(): self.relayActionData = '1'
                        else: self.relayActionData = '0'
                    else:
                        if self.btn_Ui[group][i].isChecked(): self.relayActionData = self.relayActionData + ',1'
                        else: self.relayActionData = self.relayActionData + ',0'
            ...
            for i in range(24):
                if i == 0:
                    if self.btn_Ui[groupId-1][i].isChecked(): self.relayActionData = '1'
                    else: self.relayActionData = '0'
                else:
                    if self.btn_Ui[groupId-1][i].isChecked(): self.relayActionData = self.relayActionData + ',1'
                    else: self.relayActionData = self.relayActionData + ',0'
    ...
    def DRG116_Action(self, groupId):
        if groupId == 0:
            for group in range(4):
                ...
                self.rtSerialData = sovico_DRG116(group+1, self.relayActionData)
                try:
                    self.serNum2.write(binascii.unhexlify(self.rtSerialData.drg116()))
                    sleep(.1)
                except:
                    pass
        else:
    ...
    def bssMeterReturn(self):
        try:
            ...
            start_new_thread(self.udpReturnThread, ('m,{},{},!'.format(LocalID,rtData), ))
            start_new_thread(self.Communicate_State,(True,))
        except:
    ...
    def udpReturnThread(self, rtData):
        ...
        self.sock.sendto(rtData.encode('utf-8'),(clientServerIP,int(clientServerPort)))
        self.sock.close()
            ...
            if self.ser.isOpen():
                try:
                    ...
                    if self.serResponse == '':
                        ...
                        self.bluMeterPresence = self.re_P.search(self.serResponse)                  
                        self.levelAddr = int(self.bluMeterPresence.group(1))
                        self.levelValue = self.bluMeterPresence.group(2)
                        print(self.bluMeterPresence.group(1),self.bluMeterPresence.group(2))
...
class audioStreamReceiver(QThread):
    ...
    def __init__(self, parent = None):
        ...
        self.audio_file = ['1.wav','2.wav','3.wav','4.wav','5.wav','6.wav','7.wav','8.wav']
    ...
    @Slot(int)
    def play(self, index):
        playmedia = "/home/page/paging/audio/"+self.audio_file[index]     #Set audio file folder
        print(playmedia)
